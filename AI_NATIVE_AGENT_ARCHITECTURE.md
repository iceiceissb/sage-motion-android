...
当前工程已经具备安全远程主模型链路，以及由服务端强制执行的 skill / tool 权限层；只有配置部署后的后端地址时才启用。正式实验仍使用可复现的确定性 runtime：
...
| 视觉发现 | 照片、圈选区域、问题 | ML Kit + Terra 多模态图像输入 | 可核查的视觉线索与问答 |
| 语音陪伴 | 转写文本、当前路线 | Vosk、地点、天气、TTS | 上下文回答与播报 |
| 动态改道 | 封路、降雨、人流等变化 | 高德路线、天气 | 新旧路线权衡 |
| 旅程编排 | 路线、照片、问答、行为 | 本地记忆、渲染器 | 知识游记与分享素材 |

Skill 现在由服务端按场景选择，并同时决定受信任指令与 tool allowlist；同一白名单在 schema 暴露和实际执行时各校验一次。Tool 是有 schema、超时、权限、来源和失败语义的代码接口。Skill 不是客户端可指定的自由文本，也不等于子 Agent。
...
- 端侧照片与语音默认本机处理。B1 只有在用户逐次勾选授权后才把最长边 1280 px、最多 1.2 MB、去除元数据的 JPEG 发送给后端；授权不持久化，图片不写入日志或数据库，Responses 请求使用 `store: false`。
...
2. **已完成：首批工具与视觉协议。** Environment、Place 与路线画像工具已有 strict schema、超时、来源与失败语义；Vision 已接通获授权的多模态图片输入并保留 ML Kit 端侧回退，Speech、Memory、Journey 仍保留端侧/后续扩展边界。
3. **已完成：可执行 skill 层。** A/B/B1/B2/C/D 各自映射到具名 skill；skill 对提示和工具执行进行服务端约束。
4. **已完成：服务端与 `RemoteAgentApi`。** 使用 SSE 实现现有 `AiDemoApi` 协议，客户端不保存模型密钥。
5. **已完成：接入一个多模态主模型。** Responses API 已覆盖当前 A/B1/B2/C/D 任务解释；B1 可直接输入获授权的照片。默认使用支持图像输入的 `gpt-5.6-terra`，保留 ML Kit、高德和确定性评分作为端侧事实/降级路径；部署后端并设置密钥后启用。
6. **建立评测。** 检查工具选择正确率、无数据时拒绝虚构、路线事实一致性、端到端延迟和中断恢复。
7. **按收益引入子 Agent。** 只在游记生成或跨来源研究证明并行收益后启用。
