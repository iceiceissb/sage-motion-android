...
最新实现与验证见 [2026-09-19 功能补齐说明](FEATURE_COMPLETION_2026-09-19.md)。已补圈选裁剪与问答接入、真实路线绑定、照片持久保存、定位记录与导出、生成式纸刊接口。云端能力仍需配置后端；相机、GPS、语音和分享需真机验收。此前问题基线见 [功能走查](FUNCTIONAL_AUDIT_2026-09-18.md)。
...
模型 API Key 只由 `backend/` 读取服务端环境变量，Android 仅持有后端 URL 和受控环境使用的客户端令牌。
地图、定位、步行路线几何与原生 `Polyline` 始终由高德 SDK 提供；主模型只解释约束与工具事实，不生成或替换高德路径。
后端的启动、部署、安全边界与 Android 接线见 [backend/README.md](backend/README.md)。
...
├─ data/agent/ParkAgentRuntime.kt # 正式实验/远程主 Agent/本地降级选择
├─ data/agent/RemoteAgentApi.kt # 后端 SSE 适配器，不含模型密钥
...
- JVM 单元测试：slim / full 各 24 项通过，含回答流提前结束时的本地回退。
- 后端测试：22 项通过（含 Responses 请求契约、鉴权、SSE、skill/tool 双层白名单与生产配置校验）。
- Debug APK 构建：通过。
- Android Lint：0 错误、3 个现有警告（ExifInterface 和 ChromeOS ABI）。
- Compose 页面渲染：24 张，含 390 × 844dp 主要页面及 360 × 740dp、1.15 倍字体的路线结果和探索工作台。截图中的地图为预览资源，不替代真机定位、相机、语音和高德服务验证。
